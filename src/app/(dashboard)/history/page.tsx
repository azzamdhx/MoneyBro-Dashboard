...
export default function HistoryPage() {
  ...
  const { data, loading } = useQuery<HistorySummaryData>(GET_HISTORY_SUMMARY, {
  ...
  const cashFlowData = [
    ...
    { name: "Pengeluaran", value: expenseSummary?.total || 0, fill: "url(#gradient-expense)", gradientColors: ["#fa709a", "#fee140"] },
    { name: "Cicilan", value: payments?.totalInstallment || 0, fill: "url(#gradient-installment)", gradientColors: ["#667eea", "#764ba2"] },
    { name: "Hutang", value: payments?.totalDebt || 0, fill: "url(#gradient-debt)", gradientColors: ["#f093fb", "#f5576c"] },
  ];
  ...
  const previousPaymentsChartData = [
    ...(payments?.installments?.map((item, index) => ({
      name: item.name, value: item.amount, type: "Cicilan",
      fill: `url(#gradient-previous-${index})`, gradientColors: ["#667eea", "#764ba2"],
    })) || []),
    ...(payments?.debts?.map((item, index) => ({
      name: item.personName, value: item.amount || 0, type: "Hutang",
      fill: `url(#gradient-previous-debt-${index})`, gradientColors: ["#f093fb", "#f5576c"],
    })) || []),
  ].slice(0, 8);
  ...
  return (
    <div className="space-y-6">
      ...
      <HighlightCard
        ...
        balanceLabel={
          <p className="text-xs sm:text-sm font-medium text-white/70 hidden sm:block">
            {currentMonth ? formatMonthYear(`${currentMonth}-01`) : ""}
          </p>
        }
        balanceValue={
      ...
      <div className="grid grid-cols-2 gap-3 md:hidden">
        <Link href="/incomes">
          <Card className="h-full transition-colors hover:bg-muted/50 py-4 gap-4">
            ...
            <CardContent className="px-4">
              <div className="text-md font-bold text-income break-all">
        ...
        <Link href="/expenses">
          <Card className="h-full transition-colors hover:bg-muted/50 py-4 gap-4">
            ...
            <CardContent className="px-4">
              <div className="text-md font-bold text-expense break-all">
